...
- Entering a title into the search bar will render the list down and show the  related title. Pressing the reset button will refresh the page. - The users journey begins at the Home page. The navbar is at the top of the page with the logo on the left and the Home, Sign-In and Register options on the right. There is a blured image of a bedroom in the background. The "Manga List" header is just below the navbar and underneath is Search bar and a list of manga titles. ![Search](readme.files/search.png)
- The search bar has an text input box  with a red "Reset" and blue "Search" button on the right.
...
- Unrelated words or titles will return a message "No Results Found". - The users journey begins at the Home page. The navbar is at the top of the page with the logo on the left and the Home, Sign-In and Register options on the right. There is a blured image of a bedroom in the background. The "Manga List" header is just below the navbar and underneath is Search bar and a list of manga titles. ![Home page](readme.files)
- The search bar has an text input box  with a red "Reset" and blue "Search" button on the right.
- Entering a title into the search bar will render the list down and show the  related title. Pressing the reset button will refresh the page.
...
- To register for an account users can press "Register" located on the navbar to be directed to the registration page. There are 2 input boxes for users to choose a username and a password of at least 5 characters. Below the input boxes is a green "Register" button. In the event the user wanted the Sign page, there is a prompt below the "Register" button that says "Already register Sign In" Pressing Sign In will redirect the user to the Sign In page. ![Registration page](readme.files)
- If the user does not enter the required amount of characters, the line will turn red. Users will need to click the "Register" button to complete their registration. ![Registration requirements](readme.files)
- If the username exist the page will be refreshed displaying the message "Username already exists" ![existing_user](readme.files)
- A successful registration will direct users to the profile page displaying the message "Registration Successful!" The profile page features a card displaying the message "'Username's Profile".![success_reg](readme.files)
- The Sign In page is very similar to the Registration page. There are 2 input boxes to enter the username and password and a green button that says "Sign In". Below the button there is a prompt "New here? Register". Pressing Register directs users to the registration page. ![signin page](readme.files)
- To complete a sign in users must click on the Sign In button
- Text input areas left blank will turn the area red as both areas are required for a successful sign in. ![signin_requirements](readme.files)
- If the user does not enter the correct username or password then a message "Incorrect Username and/or Password entered" ![incorrect_user](readme.files)
- A successful sign in will direct the user to the profile page with the message "Welcome {username}". ![profile_page](readme.files)
- Once a user has signed in the navbar options will update to Home, Profile, Add Title and Sign Out in the navbar. ![reg_user_menu](readme.files)
...
- Below the story section registered users will see a blue button "Write a Review" of that title. ![write_rev_btn](readme.files)
- If the user is the creator of the entry then they will see a green "Edit" and a red "Delete" button. ![edit_del_btn](readme.files)
- Pressing the "Write a Review" button triggers a model with 2 input boxes, "Name Your Review" and "Write a Review". Below is a red "Cancel" button and a green "Add Review" button. ![rev_modal](readme.files)
...
- Pressing the green "Add Review" button directs users tot he Home page displaying a message "Review Successfully Added" ![rev_add_success](readme.files)
- If the user goes back to that titles summary page they will see that their review has been added to the review section with an  "Edit" and "Delete" button . ![added_review](readme.files)
- If a user attempts to enter a 2nd review on the same title they will be redirected to the Home page with the message "You have already reviewed this title." ![existing_review](readme.files)
- Pressing the green "Edit" button in the Summary section opens up a model that is pre populated with the titles existing data. At the bottom of the modal is a red "cancel" button that refreshes the page and a green "complete" button that submits the changes the user has made. ![edit_title](readme.files)
- Once the user has pressed the "complete" button they are redirected to the Home page with a the message "Title Successfully Updated". ![update_title_mes](readme.files)
- Pressing the red "Delete" button in the summary section opens up a modal that warns the user "{title} will be permanently deleted". At the botttom of the modal is a red "No" button and a green "Yes" ![del_title](readme.files)
- Pressing "No" refreshes the page.
- Pressing "Yes" redirects the user to the Home page with a message "Title Successfully Deleted". ![del_title_mess](readme.files)
- In the review section if the user presses the green "Edit" button then a modal is opened that is prepopulated with the review data and a red "Cancel" button and a green "Complete" button. ![edit_rev](readme.files)
- The cancel button will refresh the page and the complete button will redirect the user to the Home page with the message "Review Successfully Updated" ![edit_rev_mess](readme.files)
- Pressing the red "Delete" button in the Review section opens up a modal that warns the user "Review will be permanently deleted". At the botttom of the modal is a red "No" button and a green "Yes" ![del_rev](readme.files)
- Pressing "No" refreshes the page.
- Pressing "Yes" redirects the user to the Home page with a message "Review Successfully Deleted". ![del_rev_mess](readme.files)
- Users who press the Add Title in the navbar will be directed to the Add title page. They will have to complete 6 text input areas. Name of title, Image (url), Year of release, Status, Mangaka (Author) and Story. At the bottom of the page is a green "Add Title" button and a red "Cancel" button. ![add_title_page](readme.files)
- All sections need to be completed in order to submit a new title otherwise the input area turns red. ![add_title_req](readme.files)
- If the user pressed the cancel button they are redirected to the Home page. If they press the Add Title button they are redirected to the Home page with the message "Title Successfully Added". ![add_title_mes](readme.files)
- If a user submits a title that already existing the add title page will refresh with a message "Title alreday exists" ![exist_title_mess](readme.files)
- If the user presses Sign Out on the navbar they are directed to the Sign In page with the message "You have signed out" ![signout_mess](readme.files)
- Pressing on the Home button directs users to the Home page.
- If the user clicks on the logo the Home page is refreshed. This is the same on every page.
- The user is directed to the Summary page when a title from the list is clicked. ![Press title](readme.files)
- The Summary page has a "Summary" header and below is a card that shows an image of the related title, Name of title, Year of Release, Status, Mangaka (author) and the Storyline. The image will be enlarged if the user clicks on the image. Users can get back to the home page if they press the back button located at the top left-hand side of the page. When the user scrolls further down the page they can see reviews left by registered users. ![Summary page](readme.files)
